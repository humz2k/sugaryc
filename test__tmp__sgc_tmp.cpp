...
inline static string_list make_string_list(void)
{
  string_list out = (string_list) GC_malloc(sizeof(struct string_list_struct));
  out->raw_list = make_void_list();
  return out;
}

inline static string *__index__(string_list my_list, int *idxs, int nidxs)
{
  return (string *) __index__(my_list->raw_list, idxs, nidxs);
}

inline static int append(string_list my_list, string item)
{
  return append(my_list->raw_list, (void *) item);
}

inline static string pop(string_list my_list)
{
  return (string) pop(my_list->raw_list);
}

inline static string pop(string_list my_list, int idx)
{
  return (string) pop(my_list->raw_list, idx);
}

inline static int insert(string_list my_list, int idx, string item)
{
  return insert(my_list->raw_list, idx, (void *) item);
}

inline static int len(string_list my_list)
{
  return len(my_list->raw_list);
}

inline static string __str__(string_list my_list)
{
  string out = "[";
  int length = len(my_list);
  for (int i = 0; __lt__(i, __sub__(length, 1)); i++)
  {
    out = __add__(out, __add__(to_str(__index_wrapper__(my_list, i)), ", "));
  }

  if (__gt__(length, 0))
  {
    out = __add__(out, to_str(__index_wrapper__(my_list, __sub__(length, 1))));
  }
  out = __add__(out, "]");
  return out;
}

__global__ void my_kernel(int3 blockIdx, int3 blockDim, int3 threadIdx, int s)
{
  int i = __add__(threadIdx.x, __mul__(blockDim.x, blockIdx.x));
