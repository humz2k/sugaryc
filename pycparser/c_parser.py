...
class CParser(PLYParser):
    ...
    def p_postfix_expression_4(self, p):
        """ postfix_expression  : postfix_expression PERIOD ID LPAREN RPAREN
                                | postfix_expression PERIOD ID LPAREN argument_expression_list RPAREN
        ...
        """
        if (len(p) >= 6):
            func_name = c_ast.ID(p[3], self._token_coord(p, 3))
            if (len(p) == 6):
                p[0] = c_ast.FuncCall(func_name, c_ast.ExprList([p[1]], p[1].coord), p[1].coord)
                p[0].sugared = True
                return
            else:
                expr_list = p[5]
